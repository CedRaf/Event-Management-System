  ...
  const markAsRead = async (notification) => {
    try {
      const response = await axios.patch(
        `http://localhost:3000/notifications/markAsRead`,
        {
        ...
        setNotifications((prevNotifications) =>
          prevNotifications.map(
            (notification) =>
              ...
              response.data.updatedNotification.notificationID
  ...
  const deleteNotification = async (notification) => {
    try {
      const response = await axios.delete(
        `http://localhost:3000/notifications/deleteNotification/${notification.notificationID}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (response && response.data) {
        ...
        setNotifications((prevNotifications) =>
          prevNotifications.filter(
            (notification) =>
              notification.notificationID !==
              response.data.deletedNotification.notificationID
          )
        );
      }
    } catch (err) {
